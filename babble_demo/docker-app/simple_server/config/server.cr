...
Lucky::Server.configure do |settings|
  if Lucky::Env.production?
    ...
    settings.port = ENV["DEV_PORT"].to_i
  else
    settings.secret_key_base = "pOrbY1O3Ks/Qdqqzo8ImLU19e7aZR4X2ScLfj39joeU="
    # Change host/port in config/watch.yml
    # Alternatively, you can set the DEV_PORT env to set the port for local development

    settings.host = "0.0.0.0" # not used
    settings.port = 5000      # not used
  end
